...
class ArticleLabel extends Component{
	...
	tableModule(){
		...
		const { 
            list=[
                {id:0,ass_lab_code:'0001111',ass_lab_name:'阿斯顿发生',main:'水电费水电费'},
                {id:1,ass_lab_code:'0001111',ass_lab_name:'阿斯顿发生',main:'大范甘迪发'}
            ],
            total=2
        } = listData;
		let columns = [
			{
    			...
    			width:'27%',
    			render: text => {
    				let { ass_lab_code='' } = text;
    				return(
    					<span>{ass_lab_code}</span>
    				)
    			}
			},
			{
    			...
    			width:'27%',
    			render: text => {
    				let { ass_lab_name='' } = text;
    				return(
    					<span>{ass_lab_name}</span>
    				)
    			}
			},
            {
