...
class AppletsIconAdd extends Component{
    ...
    handleSave = (callback) => {
        ...
        if(this.isSubmit){
            this.isSubmit = false;
            let validateList = ['name','picture','jump_type','order','status'];
            if(jumpType != 2){
    ...
    nameValidator = (rule, value='', callback) => {
        // 定义校验规则
        let reg =  /^[A-Za-z0-9\u4e00-\u9fa5]{1,5}$/gi
        // 去掉空格之后再校验，防止空格占位
        let str = value.trim();
        if (!reg.test(str)) {
            this.isSubmit = true;
            callback('只能是中文、字母、数字，输入长度不可超过5个字符');
        }else{
            callback();
        }
    }
    ...
    formModule(){
		...
		const { token='' } = data;
        const { id,name,picture,jump_type,jump_appid,jump_link,order,status } = onceData;
        const formList = [
            {
                type:'Input',
				label:'icon名称',
				...
				domOptions:{placeholder:"请输入icon名称"}
            },
            {
                type:'Dom',
				label:'icon图',
			...
			},
            {
				...
				domOptions:{placeholder:"请输入icon顺序"}
            },
            {
                type:'Select',
                label:'icon状态',
				...
				domOptions:{placeholder:'请选择icon状态'},
				optionList:[
                    {val:0,label:'上架'},
                    {val:1,label:'下架'}
                ],
