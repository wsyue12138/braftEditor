...
class UnknownQuestion extends Component{
	...
	getList = () => {
		...
		dispatch({
			type:'unknownQuestion/fetchGetUnknownQestion',
	...
	//抽屉
	drawerModule(){
		const { unknownQuestion:{drawerType,visible} } = this.props;
		...
		if(drawerType === 'similar'){
			drawerOptions = {
				content:(<SimilarModule {...this.props} getList={this.getList} />),
				onCancel:this.handleClose,
				isOk:false
			}
			title = '未知问题学习/关联相似';
...
export default UnknownQuestion;