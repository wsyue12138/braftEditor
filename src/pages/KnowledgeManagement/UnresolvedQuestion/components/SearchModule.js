...
class UnresolvedQuestionSearch extends Component{
	...
	handleReset = () => {
		const { form:{resetFields},handleSearch } = this.props;
		resetFields();
		handleSearch();
	}
	...
	exportModule(){
		...
		const options = {
			action:'/icservice/UnresolvedQestion/exportUnresolvedQuestion',
			headers:{
	...
	render(){
		const searchList = [
			...
			},
			{
				...
				btnTitle:'查询',
				iconType:'search',
				domOptions:{type:"primary",onClick:this.handleSearchData}
			},
			{
				type:'Button',
				btnTitle:'重置',
...
export default UnresolvedQuestionSearch;